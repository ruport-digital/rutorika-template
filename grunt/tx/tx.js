...
function criticalModernizr(grunt, project) {
  ...
  const modernizrPath = (`${project.build.dir}${html.match(/src=".*Modernizr\/.*?"/gm)}`).replace(/"|'|src=/gm, '');
...
function dataURICleanup(grunt, project, helpers) {
  const scss = grunt.file.read(`${project.res.css.sass}${helpers.scss}${helpers.dataURISCSS}`).replace(/"/gm, '').replace(/\t/gm, '  ').replace(/\n\n$/gm, '\n');
